...
function index({news,fullData,totalNews,totalPages,currentPage,searchSlug}) {
  ...
  const [initialScreening,setInitialScreening]=useState(true)
    ...
    const inputFieldReset=()=>{
      setBrandInput('');setInputFieldTouched(false);setInitialScreening(true)
    }
...
{((brandOptions.length > 0 || tagOptions.length>0) && inputFieldTouched) && <ul className="relatedDataList">
                ...
                {brandOptions?.map((brand, index) => (
                  <Link legacyBehavior href={`/news/brand/${brand?.attributes?.slug}`}><li  className="border-bottom p-2 searchResultItem cursor_pointer" key={index}  onClick={()=>{ setBrandInput(brand?.attributes?.name);setInitialScreening(true);setInputFieldTouched(false)}}>{brand?.attributes?.name}</li></Link>
                ))}
              ...
              {(inputFieldTouched && !searchLoading ) && <span aria-hidden="true" className="fs-2 cursor_pointer" onClick={() => {inputFieldReset()}}>&times;</span>}
