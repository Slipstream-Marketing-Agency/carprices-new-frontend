...
function index({ tags, fullData, totalNews, totalPages, currentPage,pageSlug }) {
    ...
    return (
        <MainLayout pageMeta={{
            title: "Latest Car Reviews UAE: New Models, Launches, and Industry Insights - Carprices.ae",
                ...
                <div className="inputSearchContainer mt-4 position-relative">
                    ...
                    {((brandOptions.length > 0 || tagOptions.length > 0) && inputFieldTouched) && <ul className="relatedDataList">
                        ...
                        {tagOptions?.map((tag, index) => (
                            <Link legacyBehavior href={`/reviews/tag/${tagOptions[index]?.slug}`}><li  className="border-bottom p-2 searchResultItem cursor_pointer" key={index}  onClick={()=>{setBrandInput(tag?.name);inputFieldReset();}}>{tag?.name}</li></Link>
                        ))}
                                        ...
                                        <div className="news-card">
                                            ...
                                            <div className="content ">
                                                <h5 className="mt-3 BlogCardHeadingTxt">
                                                    <Link legacyBehavior href={`/reviews/${newsItem?.attributes?.slug}`}><a>{`${newsItem.attributes.title.length > 50 ? `${newsItem.attributes.title.slice(0, 60)} ...` : `${newsItem.attributes.title}`}  `}</a></Link>
                                                </h5>
