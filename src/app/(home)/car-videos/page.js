...
export async function generateMetadata() {
    ...
    return {
        ...
        description,
        alternates: {
            ...(metaData?.canonicalURL ? { canonical: metaData.canonicalURL } : { canonical: `/car-videos` }),
        },
...
export default async function CarVideosPage({ searchParams }) {
    const currentPage = parseInt(searchParams.page) || 1;
    const pageSize = parseInt(searchParams.pageSize) || 9;
    const search = searchParams.search || '';

    // Fetch all car videos (not brand-specific)
    const data = await fetchBrandVideos(undefined, currentPage, pageSize);
