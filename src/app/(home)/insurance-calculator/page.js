...
const CarInsuranceCalculator = ({ apiUrl = process.env.NEXT_PUBLIC_API_URL }) => {
    ...
    const fetchBrands = async (year) => {
        setLoading(true);
        try {
            const response = await axios.get(`${apiUrl}car-trims/${year}/brands`);
    ...
    return (
        <div className="container flex flex-col items-center p-6 max-w-lg mx-auto bg-white rounded-lg shadow-md mt-8">
            ...
            {currentStep === "year" && (
                <>            <p className="text-gray-500 mb-3 font-semibold">Select Car's model year</p>
            ...
            {currentStep === "brand" && (
                <><p className="text-gray-500 mb-3 font-semibold">Select Brand</p>
            ...
            {currentStep === "model" && (
                <><p className="text-gray-500 mb-3 font-semibold">Select Model</p>
                    <div className="flex flex-wrap gap-4">
                        {models.map((model) => (
                            <button
                                key={model.slug}
            ...
            {currentStep === "trim" && (
                <><p className="text-gray-500 mb-3 font-semibold">Select Variant</p>
                    <div className="flex flex-wrap gap-4">
                        {trims.map((trim) => (
                            <button
                                key={trim.mainSlug}
            ...
            {currentStep === "spec" && (
                <><p className="text-gray-500 mb-3 font-semibold">What is Your Vehicle's Specification ?</p>
                    <div className="flex flex-wrap gap-4">
                        <button
                            className="py-4 shadow whitespace-nowrap rounded-lg hover:bg-gray-200 text-left px-2"
