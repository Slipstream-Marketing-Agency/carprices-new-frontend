        ...
        <div className="row mb-60 wow fadeInUp" data-wow-delay="200ms">
          ...
          {blogApiData?.map((news,index)=>(
            ...
            <div className="news-img">
                <Link legacyBehavior href={`${isNews ? '/news/':'/reviews/'}${news?.attributes?.slug}`}><a>
                    {/* <img src={news.attributes.coverImage.data?.attributes.url} alt="" /> */}
                    <div className="position-relative imageContainer">
                  <Image
                    src={news?.attributes?.coverImage?.data?.attributes?.url ? news?.attributes?.coverImage?.data?.attributes?.url : altImage}
                    alt="Car"
                    layout="responsive"
                    width={300}
                    height={205}
                ...
                <div className="date">
                <Link legacyBehavior href={`${isNews ? '/news/':'/reviews/'}${news?.attributes?.slug}`}><a>{t.trending}</a></Link>
                </div>
            </div>
            <div className="content">
            <h6><Link legacyBehavior href={`${isNews ? '/news/':'/reviews/'}${news?.attributes?.slug}`}><a>{ `${news.attributes.title.length>40 ? `${news.attributes.title.slice(0,50)} ...` : news.attributes.title}`}</a></Link>
                </h6>
                ...
                <div className="author-area">
                    ...
                    <h6>{news?.attributes?.author?.data?.attributes?.name}</h6>
                    <Link className='' legacyBehavior href={`${isNews ? '/news/':'/reviews/'}${news?.attributes?.slug}`}><a>{t.postedOn} - {moment(news.attributes.createdAt).format("MMMM Do YYYY")}</a></Link>
                    </div>
                </div>
                </div>
            </div>
