    ...
    <section className="">
      <div className="">
        {categories.map((category, catIndex) => (
          <div key={catIndex} className="tw-mb-10">
            <h2 className="tw-text-lg tw-py-3 sm:tw-py-5 sm:tw-text-2xl">
              {category.heading}
            </h2>
            <hr />
            <div className="tw-hidden sm:tw-grid tw-grid-cols-10">
              <div className="tw-col-span-2 ">
                {category.sections.map((section, index) => (
                  <div
                    key={index}
                    className={`tw-text-base tw-font-bold sm:tw-py-10 sm:tw-px-4 ${
                      index % 2 === 0 ? "tw-bg-gray-50" : "tw-bg-tablegrey2"
                    }`}
                  >
                    {section.header}
                  </div>
              ...
              {data.map((carTrim, index) => (
                <div key={index} className="tw-col-span-2 ">
                  {category.sections.map((section, colIndex) => (
                    <div
                      key={colIndex}
                      className={`tw-py-10 tw-flex tw-gap-2 tw-col-span-2 tw-text-base sm:tw-py-10 ${
                        section.field === "name" ? "tw-font-semibold" : ""
                      } ${
                        colIndex % 2 === 0
                          ? "tw-bg-gray-50"
                          : "tw-bg-tablegrey2"
            ...
            <div className="sm:tw-hidden">
              {category.sections.map((section, index) => (
                ...
                >
                  <h4 className="tw-text-xs tw-font-bold tw-text-center">
                    {section.header}
                  </h4>
                  <div className="tw-grid tw-grid-cols-12 tw-py-4">
                    {data.map((carTrim, i) => (
                      <div
                        key={i}
                        className={`tw-col-span-6 tw-text-center`}
                      >
                        {typeof carTrim[section.field] === "boolean" ? (
                          carTrim[section.field] ? (
                            <Image
