...
export default function OldModel({ model }) {
  ...
  const sortedYears = allYearMainTrims.sort((a, b) => b.year - a.year).filter((trim) => Number(router.query.year) === currentRealYear ? trim.year !== currentRealYear : trim.year);
    ...
    >
      ...
      >
        <div className="old_model_container d-flex justify-content-center align-items-center w-50 mx-auto">
          <FeaturedImage width={500} height={500} src={car.featuredImage} alt="" />
          <Link
            href={`/brands/${model?.brand?.slug}/${car?.year}/${model?.slug}`}
          >
  ...
  return (
    <>
      {sortedYears.length === 0 ? null : <section className="my-2">
            ...
            <div className="container">
              <div className="tabs">
                ...
                {sortedYears.map((car) => (
                  ...
                  <label
                    className={`tab ${activeTab === car.year ? "active" : ""}`}
