...
export default function SingleBrand(props) {
  ...
  const [initialPowerValues, setInitialPowerValues] = useState([
  ...
  const filteredData = useMemo(
    () => ({
      // min: minPrice,
      // max: maxPrice,
      brand: props?.models?.models[0]?.brand?.id,
      bodyType: bodyType,
      fuelType: fuelType,
      ...
      price:
        price.length === 0 && sliderPrice.length === 0
          ...
          : price.length > 0
          ? price
          : sliderPrice[0] === initialValues[0] &&
            sliderPrice[1] === initialValues[1]
          ? []
      ...
      power:
        power.length === 0 && sliderPower.length === 0
          ...
          : power.length > 0
          ? power
          : sliderPower[0] === initialPowerValues[0] &&
            sliderPower[1] === initialPowerValues[1]
          ? []
          : [{ min: sliderPower[0], max: sliderPower[1] }],
    }),
    [
      // minPrice,
      // maxPrice,
  ...
  const filteredDataForfetching = useMemo(
    () => ({
      // min: minPrice,
      // max: maxPrice,
  ...
  const handleFetchData = async (page) => {
    ...
    if (filteredData?.fuelType?.length == 0) {
      await axios
        ...
        .then((response) => {
          setFuelTypeOptions(
            response?.data?.fuelType.map((item, index) => ({
              fuelType: item,
              value: item,
            }))
          );
      ...
      await axios
        ...
        .then((response) => {
          setTransmissionOption(
            response?.data?.transmission.map((item, index) => ({
              transmission: item,
              value: item,
            }))
          );
    ...
    if (filteredData?.displacement?.length == 0) {
      await axios
        ...
        .then((response) => {
          ...
          if (res?.max > 8000) {
            filteredSelect.push({
              value: { min: 8000, max: 50000 },
              displacementLabel: "Above 8000cc+",
            });
          }
          setFilteredDisplacementSelect(filteredSelect);
          // setResponsePrice(response.data);
          setLiveDisplacement([
            response.data.min === "" ? 1000 : response.data.min,
            response.data.max,
          ]);
  ...
  useEffect(() => {
    handleFetchData(1);
    setCurrentPage(1);
  }, [filteredData, router, filteredDataForfetching]);
  ...
  const bodyTypeString = bodyTypeCounts
    ?.map((item) => {
  ...
  const bodyTypeInfo =
    bodyTypeString?.slice(0, lastIndex) +
    " and" +
    bodyTypeString?.slice(lastIndex + 1);

  const [priceTableData, setPriceTableData] = useState(
    props.modelsTableListing?.models
      .filter((trim) => trim.minPrice !== null)
  ...
  return (
    <Layout
      pageMeta={{
        ...
        )} ${currentYear} Car Prices in UAE, Latest Models, Reviews & Specifications in UAE  - Carprices.ae`,
        description: `Explore a wide selection of ${toCamelCase(
          router?.query?.brandname
        )} ${currentYear} cars at competitive prices in the UAE. Discover expert reviews, specifications, and find authorized dealers near you for a seamless car buying experience.`,
      ...
      <div className="container my-3">
        <div className="brand_banner my-5">
          <div className="brand_banner_left_sec">
            <div className="brand_logo_image">
              <FeaturedImage
                width={100}
                height={100}
                src={props?.models?.models[0]?.brand?.image}
              />
            </div>
            <div class="cpx-brand-hero__title">
              <h1 class="cpx-brand-hero__title-primary">
                {props?.models?.models[0]?.brand?.name} Car Prices, Latest
                Models, Reviews &amp; Comparison In UAE
              </h1>
              <div class="cpx-brand-hero__title-secondary">
                Here are the prices for all New{" "}
                {props?.models?.models[0]?.brand?.name} Cars in UAE
              </div>
            </div>
          </div>
          ...
          <div className="brand_image_container">
            <FeaturedImage
              width={400}
              height={400}
              src={props?.models?.models[0]?.brand?.coverImage}
            />
          </div>
        </div>
        ...
        <div className="dynamic_price_desc mb-3">
          <h2 className="fw-bold">
            {props?.models?.models[0]?.brand?.name} UAE Cars
          </h2>
          ...
          <div className="read-more-less" id="dynamic-content">
            ...
            >
                ...
                <>
                  <p
                    dangerouslySetInnerHTML={{
                      __html: props?.models?.models[0]?.brand?.description,
                    }}
                  ></p>
                  <br />
                  <h2 className="fw-bold">
                    {props?.models?.models[0]?.brand?.name} Cars{" "}
                    {moment().format("MMMM YYYY")} Price List in UAE
                  </h2>
                </>
              )}
              <p>
                You can choose from <b>{trimsCount}</b> available{" "}
                <Link
                  href={`/brands/${props?.models?.models[0]?.brand?.slug}`}
                  className="fw-bold text-primary"
                >
                  {props?.models?.models[0]?.brand?.name}
                </Link>{" "}
                ...
                UAE line-up consists of <b>{bodyTypeInfo}</b>.{" "}
                <Link
                  href={`/brands/${props?.models?.models[0]?.brand?.slug}/${minPriceTrim?.year}/${minPriceTrim?.model?.slug}/${minPriceTrim?.slug}`}
                  className="fw-bold text-primary"
                >
                  {props?.models?.models[0]?.brand?.name}{" "}
                  {minPriceTrim?.model?.name} {minPriceTrim?.name}
                </Link>
                , starting at{" "}
                <b>
                  AED <Price data={minPriceTrim?.price} />
                </b>
                , is the most affordable model while the{" "}
                <Link
                  href={`/brands/${props?.models?.models[0]?.brand?.slug}/${maxPriceTrim?.year}/${maxPriceTrim?.model?.slug}/${maxPriceTrim?.slug}`}
                  className="fw-bold text-primary"
                >
                  {props?.models?.models[0]?.brand?.name}{" "}
                  {maxPriceTrim?.model?.name} {maxPriceTrim?.name}
                </Link>{" "}
                at{" "}
                <b>
                  AED <Price data={maxPriceTrim?.price} />
                </b>{" "}
                is the brand’s most expensive model.{" "}
                <Link
                  href={`/brands/${props?.models?.models[0]?.brand?.slug}/${maxPowerTrim?.year}/${maxPowerTrim?.model?.slug}/${maxPowerTrim?.slug}`}
                  className="fw-bold text-primary"
                >
                  {props?.models?.models[0]?.brand?.name}{" "}
                  {maxPowerTrim?.model?.name} {maxPowerTrim?.name}
                </Link>{" "}
              ...
              <div className="card">
                <DataTable
                  value={props.modelsTableListing?.models
                    .filter((trim) => trim.minPrice !== null)
                    .sort((a, b) => a.minPrice - b.minPrice)}
                  scrollable
                  scrollHeight="400px"
                  ...
                  <Column
                    ...
                    body={(rowData) => (
                      <span style={{ padding: "0px" }}>
                        {props?.models?.models[0]?.brand?.name} {rowData.name}
                      </span>
                    )}
                  ></Column>
                  <Column
                    ...
                    body={(rowData) => (
                      <span style={{ padding: "0px" }}>
                        ...
                        {rowData.minPrice === rowData.maxPrice ? (
                          <Price data={rowData.minPrice} />
                        ) : (
                          <>
                            <Price data={rowData.minPrice} /> -{" "}
                            <Price data={rowData.maxPrice} />
                          </>
                        )}
                      </span>
                    )}
          ...
          <div className="white_bg_wrapper pt-4">
              ...
              <div className="w-12 me-1">
                <span className="p-float-label">
                  <MultiSelect
                    ...
                    options={_.sortBy(fuelTypeOptions, "fuelType")}
            ...
            <div className="d-flex justify-content-center flex-wrap pt-1 bg-white mx-4">
              ...
              {bodyType.map((item) => (
                <small
                  onClick={() =>
                    setBodyType(bodyType.filter((m) => m.value !== item.value))
                  }
                  className="filter_badges"
                >
                  {item}
        ...
        <div className="row mt-2 mb-4">
          <div className="col-9">
            <div className=" px-2">
              ...
              >
                <div className="row">
                  {filteredTrims.length <= 0 && isLoading === false ? (
              ...
              <div>
                <h2 className="fw-bold mb-3">
                  {props?.models?.models[0]?.brand?.name} Cars Key Highlights
                </h2>
...
export async function getServerSideProps({ req, res, params }) {
  let bodyType, fuelType, brands, minMax, minMaxPower, models, modelsTableListing;

  try {
    const [resBrand, resBodyType, resFuelType, resBrands, resMinMax, resMinMaxPower, resModelsTableListing] = await Promise.all([
      axios.get(`${process.env.NEXT_PUBLIC_API_URL}model/by-brand/slug/${params.brandname}?&orderBy=price&isAll=1`),
      axios.get(`${process.env.NEXT_PUBLIC_API_URL}trim/body-type/list`),
      axios.get(`${process.env.NEXT_PUBLIC_API_URL}trim/fuel-type/list`),
      axios.get(`${process.env.NEXT_PUBLIC_API_URL}brands?orderBy=name&isAll=1`),
      axios.get(`${process.env.NEXT_PUBLIC_API_URL}filter/get-min-max`),
      axios.get(`${process.env.NEXT_PUBLIC_API_URL}filter/power/get-min-max`),
      axios.get(`${process.env.NEXT_PUBLIC_API_URL}model/by-brand-table-listing/slug/${params.brandname}?isAll=1`)
    ]);

    models = resBrand?.data;
    bodyType = resBodyType?.data;
    fuelType = resFuelType?.data;
    brands = resBrands?.data;
    minMax = resMinMax?.data;
    minMaxPower = resMinMaxPower?.data;
    modelsTableListing = resModelsTableListing?.data;
  } catch (error) {
    if (error.response && error.response.status === 404) {
      ...
      return {
        props: {
          error: 'There was an error fetching data. Please try again later.', // You can pass this to the component to display an appropriate message
        },
      };
    }
  }
