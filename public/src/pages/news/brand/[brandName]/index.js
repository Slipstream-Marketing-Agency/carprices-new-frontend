...
// import MobileNewsListing from "@/mobile/pages/news";
...
export default function NewsListing() {
    ...
    const fetchData = async () => {
      try {
        ...
        const newsResponse = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}blog?pageSize=15&currentPage=${page}&type=news&search=${localStorage.getItem('newsbrand')}`
        );
  ...
  const handleSortingByBrand = async (option) => {

    setSearchedBrand(option)
  };

  const handleSortingByModel = async (option) => {
    setSearchedModel(option)
  };
  ...
  return (
    <Layout pageMeta={{
      title: `Latest ${toCamelCase(router?.query?.brandName)} News in UAE - Carprices.ae`,
      description: `Find the latest ${toCamelCase(router?.query?.brandName)} news, expert reviews, and exclusive updates on CarPrices.ae`,
      ...
      <div className="container">
        ...
        <div className="row mt-2 mb-4">
          <div className="col-md-9 col-12">
            ...
            <div>
              ...
              <div className="white_bg_wrapper fs-5">
                ...
                <Link href={`/news/tag/Electrical`} className="btn filter_buttons fs-6">Electrical</Link>
                      ...
                      >
                        <div className="white_bg_wrapper news_listing_image">
                          ...
                          <div className="tag_scroll">
                            {item.tags.map((tags, index) => tags.title === "" ? "" : <Link href={`/news/tag/${tags?.slug}`} onClick={() => handleClick(tags?.id)}><small className="tag_badges">{tags.title}</small></Link>
                            )}
                          </div>
          ...
          <div className="col-sm-3 col-12 right_section hide_mobile">
            {/* <Ad300x250 /> */}
            {/* <div className="mt-3">
              <PopularCars />
            </div> */}
