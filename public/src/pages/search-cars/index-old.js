...
import MobileSearchCars from "@/mobile/pages/search-cars";
import _ from "lodash";

export default function SearchCars(props) {
  ...
  const [trimsCount, setTrimsCount] = useState(null);
  ...
  const handleBodyTypeChange = (e) => {
    ...
    // Convert the array of body types to a comma-separated string
    const bodyTypesStr = e.value.join(",");

    // Update the URL using the push method
    router.push(
      {
        pathname: router.pathname,
        query: { ...router.query, bodyType: bodyTypesStr },
      },
      undefined,
      { shallow: true }
    ); // Use shallow to avoid re-fetching, re-rendering
  };

  const handleBrandChange = (e) => {
    setBrand(e.value);

    // Convert the array of brands to a comma-separated string
    const brandsStr = e.value.join(",");

    // Update the URL using the push method
    router.push(
      {
        pathname: router.pathname,
        query: { ...router.query, brand: brandsStr },
      },
      undefined,
      { shallow: true }
    ); // Use shallow to avoid re-fetching, re-rendering
        ...
        <div className="row mt-1 mb-4 ">
          ...
          <div className="col-3 mb-3">
            <div className="sticky_filter_ad_scroll">
              <Ad300x600 dataAdSlot="9145772719" />
            </div>
          </div>
        </div>
      </div>
