...
export default function NewsListing() {
    ...
    const fetchData = async () => {
      try {
        ...
        const newsResponse = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}blog?pageSize=15&currentPage=${page}&type=review&search=${localStorage.getItem('newsbrand')}`
        );
  ...
  return (
    ...
    <Layout pageMeta={{
      title: `${toCamelCase(router?.query?.brandName)} Car Reviews: Unbiased Analysis, Ratings, and Insights - Carprices.ae`,
      description: `Discover honest and unbiased ${toCamelCase(router?.query?.brandName)} Car reviews. Read detailed reviews on the latest models, performance, features, and more.`,
