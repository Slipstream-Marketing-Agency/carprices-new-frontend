  ...
  const renderContent = () => {
    ...
    paragraphs.forEach((paragraph, index) => {
      ...
      if (index === 1 || index === 4 || index === 7 || index === 10 || index === 13 || index === 15 || index === 18 ) {
        renderedContent.push(<AdBlog dataAdSlot="7520973500" />);
      }
    }
    );
  ...
  useEffect(() => {
    axios
      .post(
        ...
        `blog/by-tags?pageSize=4&currentPage=1&orderBy=title&type=review`, { tags: idArray }
      ...
      <div className="container single_news">
        <div className="row mt-2 mb-4">
          <div className="col-xs-12 col-sm-12 col-md-8 col-lg-8">
            <Breadcrumb />
            <Ad728x90 dataAdSlot="5773541907" />
          ...
          <div className="col-xs-12 col-sm-12 col-md-4 col-lg-4 right_section">
            ...
            {idArray?.length <= 0 ? "" : <><div className="d-sm-block d-none mt-3"><Ad300x250 dataAdSlot="2567975821" /></div><RelatedBlogs type="reviews" blogs={relBlog} />
            </>}
            <div className="hide_mobile sticky_scroll">
              <div className="d-flex flex-column mt-3 ">
                <Ad300x600 dataAdSlot="6343569301" />
              </div>
            </div>
          </div>
        </div>
...
export async function getServerSideProps({ req, res, params }) {
  ...
  try {
    ...
    if (resNews.data.blog.type == "news") {
      return {
        redirect: {
          permanent: true,
    ...
    let resBlogs = await axios.get(
      process.env.NEXT_PUBLIC_API_URL + "blog/min" + "?pageSize=5&type=review"
    );
